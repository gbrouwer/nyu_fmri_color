function rois = apple_loadROITSeries(view,roiname,scanList,groupNum,varargin);
% loadROITSeries.m
%
...
for roinum = 1:length(roiname)
    ...
    elseif isnumeric(roiname{roinum}) && ((roiname{roinum} < 1) || (roiname{roinum} > viewGet(view,'numberOfROIs')))
		...
		if isempty(matchScanNum)
		  rois{end}.scanCoords = apple_getROICoordinates(view,rois{end},scanNum,groupNum);
          %disp('NOT Doing matching');
		else
		  rois{end}.scanCoords = apple_getROICoordinatesMatching(view,rois{end},scanNum,matchScanNum,groupNum,matchGroupNum);
         % disp('Doing matching');
		end
                ...
                % switch this if statement and load voxels indiviudally
                % from file
   ...
   %                 disppercent(inf);
    ...
    %                disp(sprintf('(loadROITSeries) Not loading time series (loadType=%s)',loadType));
		    ...
		    if keepNAN
              ...
              % replace with noise
              nanVoxels
              [r1,r2] = size(rois{end}.tSeries);
              rois{end}.tSeries(nanVoxels,:) = normrnd(0,0.01,sum(nanVoxels),r2);
